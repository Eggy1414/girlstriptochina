      ...
      {cityGroups.map((group) => {
        ...
        return (
          <motion.div key={group.city + group.days[0].date} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <div className="border-b border-border px-1 py-2 font-display font-bold flex items-center gap-2 mb-1">
              <span className="text-xl">{config.emoji}</span>
              <span>{config.name}</span>
              <span className="text-sm text-foreground/50 ml-auto">{config.dates}</span>
            </div>
                      ...
                      >
                        <div className="px-4 pb-4 space-y-2">
                          {day.activities.map(act => (
                            <div key={act.id} className="flex items-start gap-2 border border-border rounded-lg px-3 py-2 group">
                              <span className="text-lg mt-0.5">{TYPE_EMOJI[act.type] || "🎯"}</span>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  <span className="text-xs font-semibold text-foreground/60 border border-border rounded px-1.5 py-0.5">{act.time}</span>
                                  <span className="font-semibold text-sm">{act.title}</span>
                                </div>
                                {act.notes && <p className="text-xs text-foreground/50 mt-0.5">{act.notes}</p>}
                              </div>
                              <Button
                                variant="ghost"
                                size="icon"
                          ...
                          {addingTo === day.date ? (
                              ...
                              <div className="flex gap-1 flex-wrap">
                                {(Object.keys(TYPE_EMOJI) as Activity["type"][]).map(t => (
                                  <button
                                    ...
                                    className={`text-xs px-2 py-1 rounded-full border border-border transition-colors ${newType === t ? "bg-primary text-primary-foreground" : "hover:bg-accent/30"}`}
                                  >
                                    {TYPE_EMOJI[t]} {t}
                                  </button>
                                ))}
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              className="w-full h-8 text-xs text-foreground/50"
