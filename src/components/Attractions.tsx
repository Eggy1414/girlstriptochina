                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {attr.photoWorthy && <Camera className="h-3.5 w-3.5 text-foreground/60" />}
                    </div>
                    <p className="text-xs text-foreground/60 mb-2">{attr.description}</p>
                    <div className="flex items-center gap-1 flex-wrap">
                      <span className="text-[10px] border border-border rounded-full px-2 py-0.5">
                        {config.emoji} {config.name}
                      </span>
                      {attr.tags.map(tag => (
                        <span key={tag} className="text-[10px] border border-border rounded-full px-2 py-0.5 text-foreground/60">{tag}</span>
                      ))}
                    </div>
                  </div>
